...
chart.model = (function () {
  ...
  // ---------------------
  // The people object is available at chart.model.people.
  ...
  //     publishes a 'chart-login' global custom event.
  //   * logout()- revert the current user object to anonymous.
  //     This method publishes a 'chart-logout' global custom event.
  //
  // jQuery global custom events published by the object include:
  //   * chart-login - This is published when a user login process
  //     completes. The updated user object is provided as data.
  //   * chart-logout - This is published when a logout completes.
  ...
  completeLogin = function ( user_list ) {
    ...
    chat.join();
    $.gevent.publish( 'chart-login', [ stateMap.user ] );
  };
  ...
  people = (function () {
    ...
    login = function ( name ) {
      ...
      var sio = isFakeData ? chart.fake.mockSio : chart.data.getSio();
    ...
    logout = function () {
      ...
      $.gevent.publish( 'chart-logout', [ user ] );
    };
  ...
  // The chat object API
  // -------------------
  // The chat object is available at chart.model.chat.
  ...
  //    for 'chart-listchange' and 'chart-updatechat' global
  ...
  //    It publishes a 'chart-setchatee' global custom event.
  //  * send_msg( <msg_text> ) - send a message to the chatee.
  //    It publishes a 'chart-updatechat' global custom event.
  ...
  //    'chart-listchange' event which publishes the updated
  ...
  //
  // jQuery global custom events published by the object include:
  //  * chart-setchatee - This is published when a new chatee is
  //    set. A map of the form:
  //      { old_chatee : <old_chatee_person_object>,
  //        new_chatee : <new_chatee_person_object>
  //      }
  //    is provided as data.
  //  * chart-listchange - This is published when the list of
  ...
  //  * chart-updatechat - This is published when a new message
  ...
  chat = (function () {
    ...
    _update_list = function( arg_list ) {
      ...
      // which triggers the 'chart-setchatee' global event
...
//      console.log("listchanged");
      ...
      $.gevent.publish( 'chart-listchange', [ arg_list ] );
    };
    ...
    _publish_updatechat = function ( arg_list ) {
      ...
      $.gevent.publish( 'chart-updatechat', [ msg_map ] );
    };
    ...
    _leave_chat = function () {
      ...
      if ( sio ) { sio.emit( 'leavechat' ); } //in lib/chat.js
    };
    ...
    join_chat  = function () {
      ...
      sio = isFakeData ? chart.fake.mockSio : chart.data.getSio();
    ...
    set_chatee = function ( person_id ) {
      ...
      if ( new_chatee ) {
        //if the current chatee is the new chatee just return
        if ( chatee && chatee.id === new_chatee.id ) {
          return false;
        }
      }
      else {
        //if the proposed chatee does not exist
        new_chatee = null;
      }

      $.gevent.publish( 'chart-setchatee', { old_chatee : chatee, new_chatee : new_chatee } );
      chatee = new_chatee;
      return true;
    };
